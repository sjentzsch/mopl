...
namespace dama
{
	::std::string DamaPrimPushInterior::getName() const
	{
		return "Push Interior Object";
	}

	bool DamaPrimPushInterior::isUseful(const ::rl::math::Vector& startState, const ::rl::math::Vector& endState)
	{
	...
	bool DamaPrimPushInterior::propagate(::std::vector< ::rl::math::Vector >& vecRes, const ::rl::math::Vector& endState, ::std::vector< ::std::string >& vecEdgeAction)
	{
		dama::Timer timerIK;
		...
		if(this->dModel->debugMode)
			::std::cout << "DamaPrimPushInterior::propagate: pushing object nr. " << (subspacePushObject) << " from (" << startState(indexObject) << ", " << startState(indexObject+1) << ", " << startState(indexObject+2) << ") to (" << endState(indexObject) << ", " << endState(indexObject+1) << ", " << endState(indexObject+2) << ")" << std::endl;
		...
		*/
		...
		if(this->dModel->debugMode)
			::std::cout << "#### theta_in: " << theta_in << " (rad), resp. " << (theta_in * rl::math::RAD2DEG) << " (deg)" << ::std::endl;

		// calculate the robot pushing start position
		rl::math::Transform T1_in;
		T1_in.translation().x() = startState(indexObject) - DamaPrimPush::DIST_TO_OBJECT_XY*sin(theta_in);
		T1_in.translation().y() = startState(indexObject+1) + DamaPrimPush::DIST_TO_OBJECT_XY*cos(theta_in);
		T1_in.translation().z() = startState(indexObject+2) + DamaPrimPush::HEIGHT_OFFSET_OBJECT;
		T1_in.linear() = (
		...
		// TODO: start here with a good pose on top of the table, also do this in the other push primitives !!
		// TODO: TEST THIS !!!
		//rl::math::Vector q_ik_start1 = (rl::math::Vector(10) << 6.0, 2.0, 2.0, 80.0, 0.0, -40.0, 30.0, 120.0, 0.0, 15.0).finished() * rl::math::DEG2RAD;
		rl::math::Vector q_ik_start1 = startState.segment(0, dModel->getDof(0));
		if(this->dModel->debugMode)
		{
			::std::cout << "Calculate IK1 to T = (";
			this->dModel->printTransform(T1_in, false);
			::std::cout << ") starting at q_deg = (";
			this->dModel->printQLine(q_ik_start1 * rl::math::RAD2DEG, false, false, false, false);
			::std::cout << ")" << ::std::endl;
		}
		timerIK.start();
		bool calcInverse1a = DamaModel::calcInversePositionTaskPosture(dModel->mdl, T1_in, q_ik_start1, constraint_position_orient, this->dModel->coupleJoint1And2);
		timerIK.stop();
		...
		rl::math::Transform T2_in;
		T2_in.translation().x() = realEndState(0) - DamaPrimPush::DIST_TO_OBJECT_XY*sin(theta_in);
		T2_in.translation().y() = realEndState(1) + DamaPrimPush::DIST_TO_OBJECT_XY*cos(theta_in);
		T2_in.translation().z() = realEndState(2) + DamaPrimPush::HEIGHT_OFFSET_OBJECT;
		T2_in.linear() = (
			rl::math::AngleAxis(theta_in, rl::math::Vector3::UnitZ()) *
			rl::math::AngleAxis(this->TILT_HAND_INWARD_ANGLE, rl::math::Vector3::UnitX()) *
			rl::math::AngleAxis(this->TILT_HAND_DOWNWARD_ANGLE, rl::math::Vector3::UnitY()) *
		...
		if(this->dModel->debugMode)
		{
			...
			this->dModel->printTransform(T2_in, false);
			::std::cout << ") starting at q_deg = (";
		...
		timerIK.start();
		bool calcInverse2 = DamaModel::calcInversePositionTaskPosture(dModel->mdl, T2_in, q_ik_start2, constraint_position_orient, this->dModel->coupleJoint1And2);
		timerIK.stop();
		this->dModel->timeIK += timerIK.elapsed();
	...
	::rl::math::Transform DamaPrimPushInterior::getToolObjectTransform()
	{
		rl::math::Transform T;
