	...
	workTransform(new SoVRMLTransform())
{
	...
	this->edgesCollidingMaterial->diffuseColor.setValue(0.9f, 0.0f, 0.0f);
	...
	this->edgesFreeMaterial->diffuseColor.setValue(0.2f, 0.2f, 1.0f);
	...
	// verticesFree -> my lovely vertices, now in pure black !
	...
	this->verticesFreeMaterial->emissiveColor.setValue(0.0f / 255.0f, 0.0f / 255.0f, 0.0f / 255.0f);	// 55.0f / 255.0f, 176.0f / 255.0f, 55.0f / 255.0f
...
void Viewer::customize()
{
	this->edgesCollidingDrawStyle->lineWidth = this->model->lineWidth;
	this->edgesFreeDrawStyle->lineWidth = this->model->lineWidth;
	this->verticesFreeDrawStyle->pointSize = this->model->pointSize;
}

void
Viewer::drawConfiguration(const rl::math::Vector& q)
{
	QMutexLocker lock_(&draw_mutex);
...
Viewer::drawConfigurationEdge(const rl::math::Vector& u, const rl::math::Vector& v, const bool& free)
{
	...
	//if (free)
	bool movingObject = this->model->isMovingObject(u, v);

	//std::cout << "drawConfigurationEdge: " << u(2) << " " << u(3) << " | " << v(2) << " " << v(3) << std::endl;

	//if(u(2) == -0.9 && u(3) == -0.9 && v(2) == -0.9 && v(3) == -0.9)
	//if (free)
	if(!movingObject)
	{
		// draw TRANSIT in blue
		coordinate = this->edgesFreeCoordinate;
		indexedLineSet = this->edgesFreeIndexedLineSet;
	}
	else
	{
		// draw MANIPULATION in red
	...
	rl::math::Real steps = std::ceil(this->model->robotMovementDistance(u, v) / this->delta);
	...
	if (steps > 0)
	{
		for (std::size_t l = 0; l < this->model->getOperationalDof(); ++l)
		{
			for (std::size_t i = 0; i < steps + 1; ++i)
			{
				...
				this->model->updateFrames(false);	// set to true if you want to see robot move while searching for a solution
...
{
	// TODO: delete me if you want to draw the final path
	return;
	...
	for (std::size_t l = 0; l < this->model->getOperationalDof(); ++l)
	{
		...
		{
			rl::math::Real steps = std::ceil(this->model->robotMovementDistance(*i, *j) / this->delta);
