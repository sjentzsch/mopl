...
namespace dama
{
	::std::string DamaPrimPushFrontal::getName() const
	{
		return "Push Frontal Object";
	}

	bool DamaPrimPushFrontal::isUseful(const ::rl::math::Vector& startState, const ::rl::math::Vector& endState)
	{
	...
	bool DamaPrimPushFrontal::propagate(::std::vector< ::rl::math::Vector >& vecRes, const ::rl::math::Vector& endState, ::std::vector< ::std::string >& vecEdgeAction)
	{
		dama::Timer timerIK;
		...
		if(this->dModel->debugMode)
			::std::cout << "DamaPrimPushFrontal::propagate: pushing object nr. " << (subspacePushObject) << " from (" << startState(indexObject) << ", " << startState(indexObject+1) << ", " << startState(indexObject+2) << ") to (" << endState(indexObject) << ", " << endState(indexObject+1) << ", " << endState(indexObject+2) << ")" << std::endl;
	...
	::rl::math::Transform DamaPrimPushFrontal::getToolObjectTransform()
	{
		rl::math::Transform T;
